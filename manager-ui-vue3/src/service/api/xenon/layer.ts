...
export function getPublishableResources(
    workspaceName: string,
    datastoreName: string
) {
    // Call real API to get available (unpublished) feature types from datastore
    return request<any>({
        url: `/api/v1/workspaces/${workspaceName}/datastores/${datastoreName}/featuretypes?list=available`,
        method: 'get'
    })
        .then((response: any) => {
            // Transform API response to PublishableResource format
            const resources: PublishableResource[] = (response.featureTypes || []).map((ft: any) => ({
                name: ft.name,
                nativeName: ft.name,
