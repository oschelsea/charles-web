...
const filteredLayers = computed(() => {
  ...
  return layers.value.filter(
    layer => layer.name.toLowerCase().includes(search) || layer.title?.toLowerCase().includes(search)
  );
});
...
watch(selectedWorkspace, async newVal => {
  ...
  if (newVal) {
    ...
    try {
      ...
      datastores.value = results.filter((ds: any): ds is DataStore => Boolean(ds));
    } catch {
...
const columns: DataTableColumns<Layer> = [
  {
    ...
    render(row) {
      ...
      return h(
        ...
        [
          h('span', { class: 'text-18px' }, meta?.icon || '📑'),
          h('span', { class: 'font-600 text-[var(--n-primary-color)]' }, qualifiedName)
        ]
      );
    }
  },
  ...
  },
  {
    ...
    render(row) {
      const meta = layerTypes[row.type];
      let tagType: 'info' | 'warning' | 'default' = 'default';
      if (row.type === 'VECTOR') {
        tagType = 'info';
      } else if (row.type === 'RASTER') {
        tagType = 'warning';
      }
...
async function loadLayers() {
  ...
  try {
    ...
    const results = await Promise.all(detailPromises);
    layers.value = results.filter((l): l is Layer => Boolean(l));
  } catch {
...
<template>
  <div class="p-4">
    <NSpace vertical :size="16">
      <NCard title="图层管理" :bordered="false" size="small" class="card-wrapper sm:flex-1-hidden">
    ...
    >
      ...
      >
        ...
        <template v-if="!isCreating && editingLayer.datastoreName">
          <NFormItem label="数据存储">
            <NTag size="small" type="info" :bordered="false">
              ...
              <template v-if="editingLayer.workspaceName">({{ editingLayer.workspaceName }})</template>
            </NTag>
          </NFormItem>
        </template>

        <NFormItem label="图层名称" path="name" required>
          <NInput v-model:value="editingLayer.name" placeholder="输入图层名称" :disabled="!!editingLayer.id" />
        </NFormItem>
        <NFormItem label="标题" path="title">
          <NInput v-model:value="editingLayer.title" placeholder="输入显示标题" />
        </NFormItem>
        ...
        <NFormItem label="描述" path="description">
          <NInput v-model:value="editingLayer.description" type="textarea" placeholder="输入描述信息" />
        </NFormItem>
      ...
      <NDrawerContent :title="currentDetailLayer?.title || currentDetailLayer?.name || '图层详情'" closable>
        <NSpin :show="loadingDetail">
          <template v-if="currentDetailLayer">
            <NSpace size="small" class="mb-16px">
              <NTag
                :color="{
                  borderColor: getLayerTypeMeta(currentDetailLayer.type).color,
                  textColor: getLayerTypeMeta(currentDetailLayer.type).color
                }"
                bordered
              >
                {{ getLayerTypeMeta(currentDetailLayer.type).icon }}
                {{ getLayerTypeMeta(currentDetailLayer.type).label }}
              </NTag>
            ...
            <NDescriptions label-placement="left" :column="1" bordered size="small" class="mb-24px">
              ...
              <NDescriptionsItem label="数据存储">
                <span v-if="currentDetailLayer.datastoreName">
                  ...
                  <NTag v-if="currentDetailLayer.workspaceName" size="small" class="ml-8px">
