...
import { styleApi, styleFormats, type Style, type StyleFormat } from '@/service/api/xenon/style';
...
const filteredStyles = computed(() => {
  ...
  return styles.value.filter(style =>
    ...
    style.title?.toLowerCase().includes(search)
  );
});
...
const columns: DataTableColumns<Style> = [
  {
    ...
    sorter: 'default',
    render(row) {
      return h('span', { style: { fontWeight: '600', color: 'var(--n-primary-color)' } }, row.name);
    }
  },
  {
    title: '标题',
  ...
  },
  {
    ...
    render(row) {
      ...
      return h(NTag, {
        type: row.format === 'SLD' ? 'info' : row.format === 'CSS' ? 'success' : 'warning',
        round: true,
        size: 'small'
...
async function handleSaveContent() {
  try {
    await styleApi.updateContent(
      editingStyle.value.name!,
      editorContent.value,
      editingStyle.value.format!
    );
...
<template>
  <div class="p-4">
    <NSpace vertical :size="16">
      <NCard title="样式管理" :bordered="false" size="small" class="sm:flex-1-hidden card-wrapper">
    ...
    >
      ...
      >
        <NFormItem label="名称" path="name" required>
          <NInput
            v-model:value="editingStyle.name"
            placeholder="输入样式名称"
            :disabled="!!editingStyle.id"
          />
        </NFormItem>
        ...
        <NFormItem label="格式" path="format" required>
          <NSelect
            v-model:value="editingStyle.format"
        ...
        <NFormItem label="描述" path="description">
          <NInput
            v-model:value="editingStyle.description"
            type="textarea"
            placeholder="输入描述信息"
          />
        </NFormItem>
    ...
    <NModal
      ...
      style="width: 80%; max-width: 1000px; height: 80vh"
    >
